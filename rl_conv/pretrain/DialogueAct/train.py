...
class TrainingModule(pl.LightningModule):
    ...
    def parse_train_specific_args(parent_parser):
        ...
        parser.add_argument('--context_history', default=1)
    ...
    def create_data_loader(self, ds_path: str, shuffle=False):
        ...
        dg = DataGenerator(dir_train_set, dir_val_set,
            dir_val_set, batch_size)
        
        self.train_dl, self.val_dlr, self.test_dl = dg()

        return self.train_dl, self.val_dlr, self.test_dl 
    ...
    def total_steps(self):
        train_files = [name for name in os.listdir(self.dir_data) if os.path.isfile(name)]

        utterance_count = sum( [ sum(1 for line in open(utils.get_path(f),'r')) for f in train_files ] )

        conv_count = len(train_files)

        train_size = utterance_count - conv_count*self.context_history

        return ( train_size // self.batch_size*self.accumulate_grad_batches ) * self.tparams.max_epochs

    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.tparams.lr)
        lr_scheduler = transformers.get_cosine_with_hard_restarts_schedule_with_warmup(
                    optimizer,
                    num_warmup_steps=self.tparams.warmup_steps,
...
class DaNet(nn.Module):
    ...
    def __init__(self, freeze_transformer=True, dropout=0.1, 
        base_model='distilbert-base-cased', model_name="DaNet", **kwargs):
        ...
        self.transformer = utils.load_pretrained_transformer(base_model, transformer=True, tokenizer=False)['transformer']
    ...
    def parse_model_specific_args(parent_parser):
        ...
        parser.add_argument('--base_model', default='distilbert-base-cased', required=False)
...
class DataLoaderGenerator():
    ...
    def __init__(self, dir_train_set, dir_val_set,
                    dir_test_set, batch_size, workers):

        self.dir_train_set = dir_train_set
        self.dir_val_set = dir_val_set
        self.dir_test_set = dir_test_set

        #self.tokenizer = BertTokenizer.from_pretrained('./models/bert-base-cased', max_length=512, padding_side='right' )
        self.tokenizer = utils.load_pretrained_transformer('distillbert-base-cased',transformer=False, tokenizer=True)['tokenizer']
    ...
    def prepare_datasets(self):
        ...
        for _dir in dir_sets:
            dataloaders.append(self.prepare_dataset(_dir))
        
        return dataloaders

    def prepare_dataset(self, dir_dset):
        ...
        li_dsets = [ SingleDataset(_f, self.tokenizer, self.target_binarizer ,self.context_history) for _f in files ]
        concat_dset = torch.utils.data.ConcatDataset(li_dsets)
        dataloader = torch.utils.data.DataLoader(concat_dset, batch_size=self.bs, shuffle=True,
            num_workers=self.workers, collate_fn=default_collate)
...
class SingleDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, file_path, tokenizer, target_binarizer, context_history  ):
        self.fp = file_path
    ...
    def __len__(self):
        return len(self.data) - self.context_history
    
    def __getitem__(self, index):
        datum = self.data[index:index+1+self.context_history]
        speaker, utterances, da = datum.T.values
        
        encoded_input = self.encode_tokenize( utterances.tolist() )
        binarized_target = self.target_binarizer.transform( da[-1:] )

        map_datum = {**encoded_input, 'da':torch.from_numpy(binarized_target) }
        return encoded_input, binarized_target
...
def main(tparams, mparams):
    ...
    trainer = pl.Trainer.from_argparse_args(tparams, progress_bar_refresh_rate=5,
                     ...
                     fast_dev_run=True,log_gpu_memory=True  )
