...
def _mk_pphrase(li_df_conv, paraphrase_counts, model, tokenizer, dir_dset, ds_name, fn):
    ...
    # list of das to ignore
        #1)
        idxs_to_pp = df[ df.apply(lambda row: all( (da != '') and ((da in li_da_labels_to_pp) or (da not in li_das_not_to_pp )) for da in (row['da'] if row['da']!=None else '').split(' ')  )  , axis=1) ].index.tolist()
