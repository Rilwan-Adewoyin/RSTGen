...
def main( batch_process_size=20,
    ...
    dir_save_dataset = utils_nlg.get_path("./dataset_v3_1/",_dir=True)
    
    # setting up source subreddit data
    dirs_rst_conv = "./dataset_v3/"
...
def edu_segmenter(li_dict_rsttext):
    ...
    for idx in range(len(li_dict_rsttext)):
        ...
        li_pos_edu = sorted( li_pos_edu, key= lambda item: int(item[0]) )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-bps','--batch_process_size', default=1200,
                             help='',type=int)        
