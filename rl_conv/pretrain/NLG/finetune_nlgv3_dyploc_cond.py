...
class NLG(nn.Module, utils.GenerationMixin42_gpt):
        ...
        **kwargs ):
        ...
        self.nlg_tokenizer = NLG_tokenizer(base_model_name,
                                os.path.join( ("./models"), f"{model_name}_tokenizer"),
                                 ...
                                 prefix_prompt_len=prefix_prompt_len,
                                 prefix_claim_len=prefix_claim_len,
                                 **kwargs)
        ...
        self.prefix_prompt_len = prefix_prompt_len
        self.prefix_claim_len = prefix_claim_len

            # prompt
        self.prefix_claim_qpe = nn.Embedding(self.prefix_claim_len, self.transformer.config.n_embd )
        self.prefix_claim_ln1 = nn.LayerNorm(self.transformer.config.n_embd)

        self.prefix_claim_embed_act = nn.GELU()
        self.prefix_claim_do = nn.Dropout(0.1)

        self.prefix_claim_ln2 = nn.LayerNorm(self.transformer.config.n_embd)
        self.prefix_claim_mha1 = nn.MultiheadAttention(self.transformer.config.n_embd, 4, dropout=0.1,
                                                            bias=True, add_zero_attn=True,
                                                            batch_first=True )
        self.prefix_claim_mlp1 = nn.Linear(self.transformer.config.n_embd, 512, bias=False)                                                            
        self.prefix_claim_mlp2 = nn.Linear(512, self.transformer.config.n_embd, bias=False)                                                            

            #claim
        self.prefix_prompt_qpe = nn.Embedding(self.prefix_prompt_len, self.transformer.config.n_embd )
        self.prefix_prompt_ln1 = nn.LayerNorm(self.transformer.config.n_embd)

        self.prefix_prompt_embed_act = nn.GELU()
        self.prefix_prompt_do = nn.Dropout(0.1)

        self.prefix_prompt_ln2 = nn.LayerNorm(self.transformer.config.n_embd)
        self.prefix_prompt_mha1 = nn.MultiheadAttention(self.transformer.config.n_embd, 4, dropout=0.1,
                                                            bias=True, add_zero_attn=True,
                                                            batch_first=True )
        self.prefix_prompt_mlp1 = nn.Linear(self.transformer.config.n_embd, 512, bias=False)                                                            
        self.prefix_prompt_mlp2 = nn.Linear(512, self.transformer.config.n_embd, bias=False)

        self.prefix_prompt_qpe.weight.data.normal_(mean=0.0, std=0.0002)
        for w in self.prefix_prompt_mha1.parameters():
            w.data.normal_(mean=0.0, std=0.0002)
        self.prefix_prompt_mlp1.weight.data.normal_(mean=0.0, std=0.0002)
        self.prefix_prompt_mlp2.weight.data.normal_(mean=0.0, std=0.0002)

        self.prefix_claim_qpe.weight.data.normal_(mean=0.0, std=0.0002)
        for w in self.prefix_claim_mha1.parameters():
            w.data.normal_(mean=0.0, std=0.0002)
        self.prefix_claim_mlp1.weight.data.normal_(mean=0.0, std=0.0002)
        self.prefix_claim_mlp2.weight.data.normal_(mean=0.0, std=0.0002)
    ...
    def parse_model_specific_args(parent_parser):
        ...
        parser.add_argument('--model_name', default='NLG_v3_dyploc_cond', required=False)
        
        parser.add_argument('--context_len', type= lambda x: eval(x), default={'rst':18, 'topics':35 } )
                       
        parser.add_argument('--max_input_len', type=int, default=229)
        
        parser.add_argument('--prefix_prompt_len', type=int, default=7)

        parser.add_argument('--prefix_claim_len', type=int, default=7)
    ...
    def forward_embedding(self, input_):
        ...
        x = self.prefix_prompt_qpe( torch.arange(self.prefix_prompt_len, dtype=torch.long, requires_grad=False, device=input_embeds_prefix_prompt.device) )
        x = einops.repeat(x, 'h w -> b h w', b = input_embeds_prefix_prompt.shape[0] )
        a, __ = self.prefix_prompt_mha1( x, input_embeds_prefix_prompt, input_embeds_prefix_prompt, need_weights=False )
        x = self.prefix_prompt_ln1( x + a )
        residual = x 
        
        x = self.prefix_prompt_do( self.prefix_prompt_embed_act( self.prefix_prompt_mlp1(x) ) )
        x = self.prefix_prompt_do( self.prefix_prompt_mlp2(x) )
        x = self.prefix_prompt_ln2( x + residual )
        ...
        if input_['prefix_claim'].numel() == 0:
            ...
            # 1 attn block with mlp output
            x = self.prefix_claim_qpe( torch.arange(self.prefix_claim_len, dtype=torch.long, requires_grad=False, device=input_embeds_prefix_claim.device) )
            x = einops.repeat(x, 'h w -> b h w', b = input_embeds_prefix_claim.shape[0] )
            a, __ = self.prefix_claim_mha1( x, input_embeds_prefix_claim, input_embeds_prefix_claim, need_weights=False )
            x = self.prefix_claim_ln1( x + a )
            residual = x 
            
            x = self.prefix_claim_do( self.prefix_claim_embed_act( self.prefix_claim_mlp1(x) ) )
            x = self.prefix_claim_do( self.prefix_claim_mlp2(x) )
            x = self.prefix_claim_ln2( x + residual )
            ...
            if x.dtype == torch.float16 and (
                ...
                clamp_value = torch.finfo(x.dtype).max - 1000
                x = torch.clamp(x, min=-clamp_value, max=clamp_value)
            input_embeds_prefix_claim = x
            
            _0 = self.prefix_prompt_len
            _1 = self.prefix_claim_len
        ...
        attention_mask[:, :_0, :_0] = input_['prefix_prompt_attention_mask_w_pad']
        attention_mask[:, _0:_0+_1, _0:_0+_1] = input_['prefix_claim_attention_mask_w_pad']
    ...
    def return_params(self):
        keys = ['base_model_name','max_input_len',
                        'scale_grad_by_freq','prefix_prompt_len', 'prefix_claim_len']
...
class NLG_tokenizer(utils.EffeciencyMixin, utils.RstTokenizerMixin):
    ...
    def __init__(self,
        ...
        self.context_len['utt'] = self.max_input_len - self.context_len_pre_utterance - self.prefix_prompt_len - self.prefix_claim_len
        ...
        self.pad_maxlens = {
            ...
            'prefix_prompt_attention_mask_w_pad':self.prefix_prompt_len,

            'prefix_claim':40,
            'prefix_claim_pos':40,
            'prefix_claim_attention_mask_w_pad':self.prefix_claim_len
    ...
    def encode(self, rst_rels, rst_ns , rst_pos, topics, topics_pos, 
        ...
        prefix_prompt = self.e2m_tokenizer( prefix_prompt, 
                            ...
                            truncation=True,
                            padding=False,
                            #max_length=self.prefix_prompt_len,
                            return_length=False,   
        ...
        prefix_prompt_attention_mask_w_pad = torch.ones( [ self.prefix_prompt_len, self.prefix_prompt_len ] )
        ...
        prefix_claim = self.e2m_tokenizer( prefix_claim, 
                            ...
                            truncation=True,
                            padding=False,
                            #max_length=self.prefix_claim_len,
                            return_length=False,   
                            return_token_type_ids=False,
                            return_attention_mask=False,
                            return_special_tokens_mask=False)['input_ids'][0]
        ...
        if len(prefix_claim)==0:
            prefix_claim_attention_mask_w_pad = torch.ones( [ 0, 0 ] )
            prefix_claim = prefix_claim.to(torch.long)
        else:
            prefix_claim_attention_mask_w_pad = torch.ones( [ self.prefix_claim_len, self.prefix_claim_len] )
        ...
        return { 'rst_start_token':rst_start_token,
                ...
                'tnsr_topics_phrase':tnsr_topics_phrase.contiguous(), 'tnsr_topics_pos': tnsr_topics_pos.contiguous(),
                 ...
                 'prefix_prompt_attention_mask_w_pad':prefix_prompt_attention_mask_w_pad,

                    #prefix claim section
                 'prefix_claim':prefix_claim,
                 'prefix_claim_pos':prefix_claim_pos,
                 'prefix_claim_attention_mask_w_pad':prefix_claim_attention_mask_w_pad                 
...
class TrainingModule(pl.LightningModule):
    ...
    @staticmethod
    def load_nlgmodel(model_name="NLG_v3_dyploc_cond", model_version=0,max_input_len=None, device="cuda:0"):
