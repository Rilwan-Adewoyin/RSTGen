...
def main(batch_size=10, mp_count=1, dset_names=['train', 'valid', 'test'], start_batches=[0,0,0], end_batches=[-1,-1,-1] ):
    ...
    dict_dname_records = json.load(
        open(os.path.join(dir_sourcedset, "wp.json"), "r"))
    ...
    for idx in range(len(dset_names)):
        ...
        if end_batch != -1:
            dict_dname_records[dset_name]['src'] = dict_dname_records[dset_name]['src'][ :batch_size*end_batch ]
            dict_dname_records[dset_name]['tgt'] = dict_dname_records[dset_name]['tgt'][ :batch_size*end_batch ]

        dict_dname_records[dset_name]['src'] = dict_dname_records[dset_name]['src'][batch_size*start_batch: ]
        dict_dname_records[dset_name]['tgt'] = dict_dname_records[dset_name]['tgt'][batch_size*start_batch: ] 
    

    for idx, dset_name in enumerate(dset_names) :
        # loading dataset
        li_records_src = dict_dname_records[dset_name]['src']
        li_records_tgt = dict_dname_records[dset_name]['tgt']

        total_batch_count = int(math.ceil(len(li_records_src) / batch_size))
        # Operating on the dataset in batches
        batches_completed = start_batches[idx]
        while len(li_records_src) > 0:
            print(f"\n\t {dset_name} dset: Operating on Batch {batches_completed}: {batches_completed-start_batches[idx]} of {total_batch_count}")
            start = time.time()
            batch_li_records = [{'prompt': p, 'reference': r} for p, r in zip(
                li_records_src[:batch_size],  li_records_tgt[:batch_size])]
            ...
            #     li_records2 = preprocess(li_records2)
            #     li_records2 = key_word_parse(li_records2)
            #     li_records2 = rst_tree_parse_records(li_records2)
            #     li_records2 = wrap_position_edus(li_records2)
            ...
            li_records_src = li_records_src[batch_size:]
