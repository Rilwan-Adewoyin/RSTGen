...
from transformers import BertConfig, BertForSequenceClassification, BertTokenizer, BertForMaskedLM, AlbertTokenizerFast, AlbertModel, AlbertForPreTraining
...
def get_cola_score(sentences):
    ...
    saved_pretrained_CoLA_model_dir = './models/cola_model/' + model_name + '/'
...
def get_coherence_score(processed_candidates):
    #Calculate Inter Sentence coherence score
    
    #Loading model
    model_name = 'albert-base-v2'
    tokenizer = AlbertTokenizerFast.from_pretrained(model_name)
    model = AlbertForPreTraining.from_pretrained(model_name).to(device)
    ...
    candidates_examples_encoded = [ tokenizer( li_pos+li_neg, **tokenizer_params) for li_pos,li_neg in zip( candidates_examples_pos, candidates_examples_neg) ]
    ...
    with torch.no_grad():
        ...
        for idx in range(len(processed_candidates) ):
            ...
            output = model(**candidates_examples_encoded[idx].to(device)) 
            log_loss = loss_fct( output.sop_logits.view(-1, output.sop_logits.size(-1)), labels[idx].to(device).view(-1) )
...
def get_gruen(candidates):
    ...
    gruen_score = [min(1, max(0, sum(i))) for i in zip(grammaticality_score, redundancy_score, focus_score, coherence_score)]
    return gruen_score, grammaticality_score, redundancy_score, focus_score, coherence_score
