...
from transformers import Adafactor, AdamW, get_cosine_schedule_with_warmup
import ujson
import yaml
...
from train_RSTGPT_arggen_dyploc import DataLoaderGenerator
...
class RSTBartDyplocConfig(RSTBart_Config):

    def __init__(self, max_len_title=22, max_len_claim=22, **kwargs):
        kwargs['model_name'] = "RSTBartDyploc"
...
class RSTBartDyploc(RSTBart):
    
    def __init__(self, config: RSTBartDyplocConfig):
    ...
    def embed(self, rst_start_token_id, rst_rel, rst_ns, rst_pos, key_phrase_ids, li_kprstpos, position_ids_kp, **kwargs ):
        ...
        embeds_claim_title = self.model.encoder.embed_tokens( kwargs.get('ids_claim_title') ) * self.model.encoder.embed_scale
                
        #appending claim and position embedding
        position_ids_claim_title = torch.cat( [kwargs.get('position_ids_claim_title') ] )
        
        position_embeds_claim_title = super(
                            type(self.model.encoder.embed_positions), 
                            self.model.encoder.embed_positions).forward(position_ids_claim_title + self.model.encoder.embed_positions.offset)
        
        claim_title_embeds = embeds_claim_title + position_embeds_claim_title
        
        inputs_embed = torch.cat([claim_title_embeds, inputs_embed],axis=1)
        return inputs_embed
    ...
    def parse_model_specific_args(parent_parser):
        ...
        parser.add_argument('--model_name', default='RSTBartDyploc', required=False)
    ...
    def load_model(cls, model_name="RSTBartDyploc", model_version=None, mparams_new={}, device="cuda:0"):
        ...
        if model_version != None:
            ...
            checkpoint = RSTBartDyploc_TrainingModule.get_ckpt_file(
            ...
            mconfig = RSTBartDyplocConfig.from_pretrained(
    ...
    def prepare_inputs_for_generation(self, decoder_input_ids, past=None, attention_mask=None, head_mask=None, 
                                      decoder_head_mask=None, cross_attn_head_mask=None, decoder_cross_attention_mask=None, use_cache=None, encoder_outputs=None, decoder_context_rstpos=None, decoder_edu_rstpos=None, **kwargs):
        
        outputs = super().prepare_inputs_for_generation(decoder_input_ids, past=past, attention_mask=attention_mask,
                                                        head_mask=head_mask, decoder_head_mask=decoder_head_mask, cross_attn_head_mask=cross_attn_head_mask, decoder_cross_attention_mask=decoder_cross_attention_mask, use_cache=use_cache, encoder_outputs=encoder_outputs, decoder_context_rstpos=decoder_context_rstpos, decoder_edu_rstpos=decoder_edu_rstpos, **kwargs)

        outputs['ids_claim_title'] = kwargs.get('ids_claim_title')
        outputs['position_ids_claim_title'] = kwargs.get('position_ids_claim_title')
...
class RSTTokenizerDyploc(RSTTokenizer):
    ...
    def encode_input(self, rst_rel, rst_ns, rst_pos, li_kp, li_kprstpos,
                     ...
                     claim='', title='', max_claim_len=None, max_title_len=None,
                     **kwargs):
        ...
        encoded['ids_claim_title'] = torch.cat( [ids_claim, ids_title] )
        ...
        encoded['position_ids_claim_title'] = torch.cat( [positions_ids_claim, positions_ids_title] )
        ...
        encoded['attention_mask'][ :claim_len , :claim_len ] = torch.ones_like(encoded['attention_mask'][ :claim_len, :claim_len ])
        encoded['attention_mask'][ claim_len:claim_len+title_len , claim_len:claim_len+title_len ] = torch.ones_like(encoded['attention_mask'][ claim_len:claim_len+title_len , claim_len:claim_len+title_len ])
        ...
        encoded['decoder_cross_attention_mask'] = torch.nn.functional.pad( encoded['decoder_cross_attention_mask'],
                                                                            (claim_title_len,0), value=1)
        encoded['decoder_cross_attention_mask'][ :, claim_len-claim_pad:claim_len ] = 0
        encoded['decoder_cross_attention_mask'][ :, claim_title_len-title_pad:claim_title_len ] = 0

        return encoded

    @classmethod
    def from_pretrained(cls,
                        dir_tokenizer="./tokenizers/RSTBartDyploc",
...
class RSTBartDyploc_TrainingModule(LightningModule):
    ...
    def __init__(self,
        ...
        self.pad_values = {'rst_start_token': mconfig.pad_token_id,
                           ...
                           'position_ids_claim_title': mconfig.pad_token_id,

                           'ids_claim_title': mconfig.pad_token_id,
        ...
        self.pad_maxlens = {
            ...
            'attention_mask': mconfig.max_len_claim + mconfig.max_len_title + mconfig.max_len_rst + mconfig.max_len_key_phrase,  # axis:max_length
            'decoder_cross_attention_mask': [ mconfig.max_len_utt, mconfig.max_len_claim + mconfig.max_len_title + mconfig.max_len_rst + mconfig.max_len_key_phrase ] , #max_lens in both 2d dimensions

            'position_ids_kp': mconfig.max_len_key_phrase,
            'position_ids_claim_title':mconfig.max_len_claim+mconfig.max_len_title,
        ...
        if self.mode in ['finetune', 'train_cont']:
            ...
            mparams_to_save = {param: getattr(mconfig, param) for param in list(filter(
                lambda p: p not in ['self','kwargs'], list(inspect.signature(RSTBartDyplocConfig.__init__).parameters.keys()) ))}
    ...
    def instatiate_training_module(tparams=None, mparams=None):
        ...
        if tparams['mode'] in ["finetune"]:            
            ...
            checkpoint = RSTBartDyploc_TrainingModule.get_ckpt_file(f"./models/RSTBart/version_{tparams['finetune_version']}/checkpoints")
            ...
            with torch.no_grad():
                model.model.encoder.embed_tokens.weight[ -2:, : ] = model.model.encoder.embed_tokens.weight[ -5:-4, : ]

            training_module = RSTBartDyploc_TrainingModule(model.config, **tparams, model=model, tokenizer=tokenizer)
    ...
    def instatiate_trainer(tparams, tb_logger, training_module):
        ...
        if tparams['mode'] in ["finetune"]:
            ...
            trainer = Trainer.from_argparse_args(argparse.Namespace(**tparams),
...
class SingleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rst_rels, rst_ns, rst_pos, li_kp, li_kprstpos, utterance, dict_pos_edu, title, claim = self.getitem_extract_datum(
            index)
    ...
    def getitem_extract_datum(self, index):
        ...
        # claim
        claim = json.loads(datum['li_claim'])[self.li_claim_idxs[index]] if self.li_claim_idxs[index] != -1 else ""
        
        title = json.loads(datum['prompt']).lstrip( string.punctuation )
...
def main(tparams={}, mparams={}):
    ...
    training_module = RSTBartDyploc_TrainingModule.instatiate_training_module(
        tparams, mparams)
    trainer, training_module = RSTBartDyploc_TrainingModule.instatiate_trainer(
        tparams, tb_logger, training_module)
    RSTBartDyploc_TrainingModule.start(trainer, tparams, training_module, mparams)
...
if __name__ == '__main__':
    ...
    mparams = RSTBartDyploc.parse_model_specific_args(parent_parser)

    # add the trainer specific args
    tparams = RSTBartDyploc_TrainingModule.parse_train_specific_args(parent_parser)
...
# CUDA_VISIBLE_DEVICES=1 python3 train_RSTBart_arggen_dyploc.py --batch_size 32 --version 1  --precision 16 --mode finetune --workers 6 --rst_tree_aligned_attention 0 --scale_grad_by_freq 1 --max_epochs 12 --gpus 1 --max_len_utt 190 --max_len_rst 28 --max_len_key_phrase 40 --tag "RSTBart with non attention" --finetune_version 1
# CUDA_VISIBLE_DEVICES=1 python3 train_RSTBart_arggen_dyploc.py --batch_size 32 --version 6  --precision 16 --mode finetune --workers 6 --rst_tree_aligned_attention 1 --scale_grad_by_freq 1 --max_epochs 12 --gpus 1 --max_len_utt 190 --max_len_rst 28 --max_len_key_phrase 40 --tag "RSTBart with rst aligned attention" --finetune_version 6
